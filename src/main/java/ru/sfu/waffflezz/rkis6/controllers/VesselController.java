...
import ru.sfu.waffflezz.rkis6.models.Vessel;
import ru.sfu.waffflezz.rkis6.services.VesselService;

/**
 * Класс представляет собой контроллер, отвечающий за обработку HTTP-запросов.
 * Он обеспечивает взаимодействие с сервисом VesselService и обработку запросов, связанных с
 * посудой, такие как создание, редактирование, удаление и просмотр.
 */
@Controller
...
public class VesselController {
  ...
  /**
   * Обрабатывает GET-запросы по пути "/vessels".
   * Если указан параметр "width", фильтрует посуду по заданной ширине.
   * В противном случае, возвращает всю посуду.
   *
  ...
  /**
   * Обрабатывает POST-запросы по пути "/vessels" для создания новой посуды.
   * Проводит валидацию посуды и сохраняет его в сервисе, если данные верны.
   *
   * @param vessel          Сущность посуды для создания.
   * @param bindingResult   Результат валидации посуды.
   * @return Представление "vessels/new" в случае ошибок валидации, иначе перенаправляет на "/vessels".
   */
  @PostMapping()
  ...
  /**
   * Обрабатывает PATCH-запросы по пути "/vessels/{id}" для обновления существующей посуды.
   * Проводит валидацию посуды и обновляет её в сервисе, если данные верны.
   *
   * @param vessel          Сущность посуды для обновления.
   * @param bindingResult   Результат валидации посуды.
   * @param id              Идентификатор посуды, которую нужно обновить.
   * @return Представление "vessels/edit" в случае ошибок валидации, иначе перенаправляет на "/vessels".
   */
  @PatchMapping("/{id}")
